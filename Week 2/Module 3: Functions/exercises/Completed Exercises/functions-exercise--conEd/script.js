...
// Coolcoolcool. Now let's see if we can pass an argument into a function. Write out a new function declaration called sendAlert(). This function will be the same as the last function except that you're going to write a parameter in the function, and pass that parameter into the alert method so that when the function is called, it's going to alert whatever argument is passed into that function call.

// First write the function declaration. Then add the parameter into the function and the alert.

const sendAlert =(runAndHide) => {
    alert (runAndHide)
}
sendAlert("What should we do?")
sendAlert("Where should we go?")
...
const randomNumber=(anyNumber) => {
    console.log(Math.floor(Math.random(anyNumber) * Math.floor(10)));
}
randomNumber(5);
...
// const soupArrLength = typesOfSoup.length;

// getLength(soupArrLength)
...
const displayLength = (length) =>{
  ...
  displayLength(soupArrLength)
  getLength(typesOfSoup)
...
const randomItem = (array)=>{
    const randomNumber = setRandomNumber(array.length);
    const randomItemFromArray = array[randomNumber];

    return randomItemFromArray;
}
const randomSoup = (randomItem(typesOfSoup));
